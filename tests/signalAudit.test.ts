...
describe('signal audit', () => {
  it('signal audit returns candidate rows', () => {
    ...
    seedCandidate(db, { tokenIdOffset: 1, mint: 'Audit111', symbol: 'EARLY', signalClass: 'EARLY_RUNNER', bestGainPct: 40, worstDrawdownPct: -10, movedBeforeDiscoveryPct: 50 });
  ...
  it('signal audit includes class counts', () => {
    ...
    const db = createDb(config);
    seedCandidate(db, { tokenIdOffset: 1, mint: 'Audit111', symbol: 'EARLY', signalClass: 'EARLY_RUNNER', bestGainPct: 40, worstDrawdownPct: -10, movedBeforeDiscoveryPct: 50 });
    seedCandidate(db, { tokenIdOffset: 2, mint: 'Audit222', symbol: 'DUMP', signalClass: 'INSTANT_DUMP', bestGainPct: 5, worstDrawdownPct: -40, movedBeforeDiscoveryPct: 20 });
    const report = buildSignalAuditReport(db, config);
