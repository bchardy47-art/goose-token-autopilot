...
const DEFAULTS = {
  ...
  PAPER_TRAILING_STOP_ENABLED: true,
  PAPER_TRAILING_ACTIVATION_PCT: 30,
  PAPER_TRAILING_STOP_PCT: 15,
  PAPER_EARLY_FADE_EXIT_ENABLED: true,
  PAPER_EARLY_FADE_MIN_HOLD_MINUTES: 30,
  PAPER_EARLY_FADE_MAX_BEST_GAIN_PCT: 15,
  PAPER_EARLY_FADE_EXIT_BELOW_PNL_PCT: -8,
  PAPER_REQUIRE_HIGH_WATCH_PRIORITY: true,
  MAX_SLIPPAGE_BPS: 500,
  MIN_LIQUIDITY_USD: 20000,
  MAX_CHASE_PCT: 150,
  MIN_TOKEN_AGE_MIN: 10,
  MAX_TOKEN_AGE_HOURS: 24,
  DEXSCREENER_RECENT_UPDATES_URL: 'https://api.dexscreener.com/token-profiles/recent-updates/v1',
  DEXSCREENER_INCLUDE_RECENT_UPDATES: true,
  DEXSCREENER_SEARCH_URL: 'https://api.dexscreener.com/latest/dex/search',
  DEXSCREENER_INCLUDE_SEARCH_PROBES: true,
  ...
  DEXSCREENER_FRESH_DISCOVERY_LIMIT: 25,
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): AppConfig {
  ...
  const config: AppConfig = {
    ...
    paperTrailingActivationPct: parseNumber('PAPER_TRAILING_ACTIVATION_PCT', env.PAPER_TRAILING_ACTIVATION_PCT, DEFAULTS.PAPER_TRAILING_ACTIVATION_PCT, { min: 0 }),
    ...
    paperEarlyFadeExitEnabled: parseBoolean('PAPER_EARLY_FADE_EXIT_ENABLED', env.PAPER_EARLY_FADE_EXIT_ENABLED, DEFAULTS.PAPER_EARLY_FADE_EXIT_ENABLED),
    paperEarlyFadeMinHoldMinutes: parseNumber('PAPER_EARLY_FADE_MIN_HOLD_MINUTES', env.PAPER_EARLY_FADE_MIN_HOLD_MINUTES, DEFAULTS.PAPER_EARLY_FADE_MIN_HOLD_MINUTES, { min: 0 }),
    paperEarlyFadeMaxBestGainPct: parseNumber('PAPER_EARLY_FADE_MAX_BEST_GAIN_PCT', env.PAPER_EARLY_FADE_MAX_BEST_GAIN_PCT, DEFAULTS.PAPER_EARLY_FADE_MAX_BEST_GAIN_PCT),
    paperEarlyFadeExitBelowPnlPct: parseNumber('PAPER_EARLY_FADE_EXIT_BELOW_PNL_PCT', env.PAPER_EARLY_FADE_EXIT_BELOW_PNL_PCT, DEFAULTS.PAPER_EARLY_FADE_EXIT_BELOW_PNL_PCT),
    paperRequireHighWatchPriority: parseBoolean('PAPER_REQUIRE_HIGH_WATCH_PRIORITY', env.PAPER_REQUIRE_HIGH_WATCH_PRIORITY, DEFAULTS.PAPER_REQUIRE_HIGH_WATCH_PRIORITY),
    ...
    dexScreenerRecentUpdatesUrl: env.DEXSCREENER_RECENT_UPDATES_URL ?? DEFAULTS.DEXSCREENER_RECENT_UPDATES_URL,
    dexScreenerIncludeRecentUpdates: parseBoolean('DEXSCREENER_INCLUDE_RECENT_UPDATES', env.DEXSCREENER_INCLUDE_RECENT_UPDATES, DEFAULTS.DEXSCREENER_INCLUDE_RECENT_UPDATES),
    dexScreenerSearchUrl: env.DEXSCREENER_SEARCH_URL ?? DEFAULTS.DEXSCREENER_SEARCH_URL,
    dexScreenerIncludeSearchProbes: parseBoolean('DEXSCREENER_INCLUDE_SEARCH_PROBES', env.DEXSCREENER_INCLUDE_SEARCH_PROBES, DEFAULTS.DEXSCREENER_INCLUDE_SEARCH_PROBES),
    ...
    dexScreenerMaxMovedBeforeDiscoveryPct: parseNumber(
      'DEXSCREENER_MAX_MOVED_BEFORE_DISCOVERY_PCT',
      env.DEXSCREENER_MAX_MOVED_BEFORE_DISCOVERY_PCT,
      parseNumber('MAX_CHASE_PCT', env.MAX_CHASE_PCT, DEFAULTS.MAX_CHASE_PCT, { min: 0 }),
      { min: 0 }
    ),
    dexScreenerFreshDiscoveryLimit: parseNumber('DEXSCREENER_FRESH_DISCOVERY_LIMIT', env.DEXSCREENER_FRESH_DISCOVERY_LIMIT, DEFAULTS.DEXSCREENER_FRESH_DISCOVERY_LIMIT, { integer: true, min: 1 }),
...
export function getConfigSafetyStatus(config: AppConfig): Record<string, unknown> {
  return redactValue({
    ...
    dexScreenerRecentUpdatesUrl: config.dexScreenerRecentUpdatesUrl,
    dexScreenerIncludeRecentUpdates: config.dexScreenerIncludeRecentUpdates,
    dexScreenerSearchUrl: config.dexScreenerSearchUrl,
    dexScreenerIncludeSearchProbes: config.dexScreenerIncludeSearchProbes,
    ...
    dexScreenerFreshDiscoveryLimit: config.dexScreenerFreshDiscoveryLimit,
