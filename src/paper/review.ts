...
export async function runPaperReview(db: AppDb, config: AppConfig): Promise<{ decisions: PaperReviewDecision[]; refreshedCount: number }> {
  ...
  try {
    ...
    db.finishRunLog(runLogId, 'SUCCESS', { reviewed: openPositions.length, refreshedCount, decisions });
    return { decisions, refreshedCount };
  } catch (error) {
